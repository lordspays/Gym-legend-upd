...
from bot.utils import format_number, pointer_to_screen_name, parse_amount_string
...
async def can_use_command(user_id: int, command_category: str) -> bool:
    ...
    if admin_level == 2:
        allowed_categories = [
            ...
            "donat_services", "info", "players", "broadcast", "halls"
        ]
        return command_category in allowed_categories
    
    if admin_level == 3:
        allowed_categories = ["main", "economy", "clans", "broadcast", "info", "halls"]
        return command_category in allowed_categories
...
def create_main_admin_keyboard(admin_level: int):
    ...
    keyboard.row()
    keyboard.add(Text("🏢 Управление залами"), color=KeyboardButtonColor.POSITIVE)
    keyboard.row()
    keyboard.add(Text("🔓 Управление доступом"), color=KeyboardButtonColor.POSITIVE)
    keyboard.row()
...
def create_logging_keyboard():
    ...
    keyboard.add(Text("🔓 Доступлоги"), color=KeyboardButtonColor.POSITIVE)
    keyboard.row()
    keyboard.add(Text("🏰 Кланлоги"), color=KeyboardButtonColor.POSITIVE)
    keyboard.add(Text("🏢 Залылоги"), color=KeyboardButtonColor.POSITIVE)
    keyboard.row()
...
def create_halls_keyboard():
    """Создание клавиатуры для управления залами"""
    keyboard = Keyboard(inline=True)
    
    keyboard.add(Text("🏢 Выдать залы"), color=KeyboardButtonColor.POSITIVE)
    keyboard.add(Text("📉 Забрать залы"), color=KeyboardButtonColor.NEGATIVE)
    keyboard.row()
    keyboard.add(Text("🔙 Назад"), color=KeyboardButtonColor.SECONDARY)
...
def create_access_keyboard():
    """Создание клавиатуры для управления доступом к Инфа"""
    keyboard = Keyboard(inline=True)
...
def create_creator_commands_keyboard():
    ...
    keyboard.row()
    keyboard.add(Text("🔄 Обновить статистику"), color=KeyboardButtonColor.POSITIVE)
    keyboard.row()
...
async def admin_main_handler(message: Message):
    ...
    main_commands = (
        ...
        "🏢 Управление фитнес-залами:\n"
        "• Выдать залы [айди] [количество] - выдать игроку фитнес-залы\n"
        "• Забрать залы [айди] [количество] - забрать у игрока фитнес-залы\n\n"
        
        "💡 Администрация имеет безграничный доступ к команде Инфа\n"
...
async def creator_help_handler(message: Message):
    ...
    creator_commands = (
        ...
        "• Обновить статистику - обновить данные после массового сброса\n"
...
@admin_labeler.message(text=["Обновить статистику", "обновить статистику"])
async def refresh_statistics_handler(message: Message):
    """Обновление статистики после массового сброса"""
    user_id = message.from_id
    ...
    try:
        ...
        # Получаем статистику по фитнес-залам
        all_players = await get_all_players()
        total_halls = 0
        for player in all_players:
            halls = await get_player_fitness_halls(player["user_id"])
            total_halls += halls
        
        recent_players = await get_recent_players(limit=5)
        ...
        await log_admin_action(
            ...
            "refresh_stats",
            0,
            f"Обновил статистику | Игроков: {total_players}, Кланов: {total_clans}, Залы: {total_halls}",
            None
        )
        ...
        stats_text = (
            ...
            f"🏢 Фитнес-залы 🏢\n"
            f" Всего залов: {format_number(total_halls)}\n"
            f" Ежедневный доход: {format_number(total_halls * 10)} монет\n\n"
            f"🏰 Кланы 🏰\n"
        ...
        return f"❌ Ошибка при обновлении статистики: {str(e)}"
...
@admin_labeler.message(text="🏢 Управление залами")
async def halls_button_handler(message: Message):
    user_id = message.from_id
    
    if not await is_admin(user_id):
        return "❌ У вас нет прав администратора!"
    
    if not await can_use_command(user_id, "halls"):
        return "❌ У вас нет доступа к этому разделу!"
    
    keyboard = create_halls_keyboard()
    
    halls_text = (
        "🏢 УПРАВЛЕНИЕ ФИТНЕС-ЗАЛАМИ\n\n"
        "• Выдать залы [айди] [количество] - выдать игроку фитнес-залы\n"
        "• Забрать залы [айди] [количество] - забрать у игрока фитнес-залы\n\n"
        "💰 Доход: 10 монет за каждый зал в день\n"
        "📊 Дневной лимит покупок: 100 залов\n\n"
        "💡 Используйте кнопки для быстрого доступа к командам"
    )
    
    await message.answer(halls_text, keyboard=keyboard)

@admin_labeler.message(text="🔓 Управление доступом")
async def access_button_handler(message: Message):
    user_id = message.from_id
    ...
    keyboard = create_access_keyboard()
    
    access_text = (
        "🔓 УПРАВЛЕНИЕ ДОСТУПОМ К КОМАНДЕ ИНФА\n\n"
        "• Доступ инфо [айди] [дни] - выдать доступ к команде Инфа\n"
        "• Доступ инфо список - список игроков с доступом к команде Инфа\n\n"
        "💡 Администрация имеет безграничный доступ к команде Инфа\n"
        "💡 Эта команда предназначена для выдачи доступа обычным игрокам"
    )
    
    await message.answer(access_text, keyboard=keyboard)
...
async def logging_button_handler(message: Message):
    ...
    logging_text = (
        ...
        "• Доступлоги - логи использования команд управления доступом к Инфа\n"
        "• Кланлоги - логи использования админ команд связанных с кланом\n"
        "• Залылоги - логи использования команд управления фитнес-залами\n"
...
# ======================
# КОМАНДЫ УПРАВЛЕНИЯ ФИТНЕС-ЗАЛАМИ
# ======================

@admin_labeler.message(text=["Выдать залы <cmd_args>", "выдать залы <cmd_args>"])
async def give_fitness_halls_handler(message: Message, cmd_args: str):
    """Выдать фитнес-залы игроку"""
    user_id = message.from_id
    ...
    if not await can_use_command(user_id, "halls"):
        return "❌ У вас нет доступа к управлению залами!"
    
    parts = cmd_args.split()
    if len(parts) < 2:
        return "❌ Укажите айди игрока и количество залов!\n📝 Использование: Выдать залы [айди] [количество]"
    ...
    try:
        halls_to_give = int(parts[1])
        if halls_to_give <= 0:
            return "❌ Количество залов должно быть положительным числом!"
    except ValueError:
        return "❌ Количество залов должно быть числом!"
    ...
    admin = await get_player(user_id)
    admin_nickname = admin.get("admin_nickname", admin["username"]) if admin else "Администратор"
    
    try:
        # Получаем текущее количество залов
        current_halls = await get_player_fitness_halls(target_id)
        
        # Выдаем залы (бесплатно, без списания баланса)
        new_halls_count = await update_fitness_halls(target_id, halls_to_give, 0)
        ...
        await log_admin_action(
            ...
            "give_halls",
            target_id,
            f"Выдал {halls_to_give} фитнес-залов | Было: {current_halls}, Стало: {new_halls_count}",
            None
        )
        ...
        return (
            f"✅ Фитнес-залы выданы успешно!\n\n"
            f"👤 Игрок: [id{target_id}|{target_username}]\n"
            f"🏢 Было залов: {format_number(current_halls)}\n"
            f"➕ Выдано: {format_number(halls_to_give)}\n"
            f"📈 Теперь у игрока: {format_number(new_halls_count)} фитнес-залов\n"
            f"💵 Ежедневный доход: {format_number(daily_income)} монет\n"
            f"👮 Выдал: [id{user_id}|{admin_nickname}]"
        )
        
    except Exception as e:
        return f"❌ Ошибка при выдаче залов: {str(e)}"

@admin_labeler.message(text=["Забрать залы <cmd_args>", "забрать залы <cmd_args>"])
async def take_fitness_halls_handler(message: Message, cmd_args: str):
    """Забрать фитнес-залы у игрока"""
    user_id = message.from_id
    ...
    parts = cmd_args.split()
    if len(parts) < 2:
        return "❌ Укажите айди игрока и количество залов!\n📝 Использование: Забрать залы [айди] [количество]"
    ...
    if current_halls < halls_to_take:
        return f"❌ У игрока недостаточно залов! Всего: {format_number(current_halls)}, требуется забрать: {format_number(halls_to_take)}"
    
    try:
        # Забираем залы (отрицательное количество)
        new_halls_count = await update_fitness_halls(target_id, -halls_to_take, 0)
        ...
        await log_admin_action(
            ...
            "take_halls",
            target_id,
            f"Забрал {halls_to_take} фитнес-залов | Было: {current_halls}, Стало: {new_halls_count}",
            None
        )
        ...
        return (
            f"✅ Фитнес-залы изъяты успешно!\n\n"
            f"👤 Игрок: [id{target_id}|{target_username}]\n"
            f"🏢 Было залов: {format_number(current_halls)}\n"
            f"➖ Забрано: {format_number(halls_to_take)}\n"
            f"📉 Теперь у игрока: {format_number(new_halls_count)} фитнес-залов\n"
            f"💵 Ежедневный доход: {format_number(daily_income)} монет\n"
            f"👮 Забрал: [id{user_id}|{admin_nickname}]"
        )
        
    except Exception as e:
        return f"❌ Ошибка при изъятии залов: {str(e)}"
...
@admin_labeler.message(text=["Доступлоги", "доступлоги"])
async def access_logs_handler(message: Message):
    user_id = message.from_id
    ...
    if not logs:
        return "📭 Логи управления доступом отсутствуют!"
    
    logs_text = "🔓 ЛОГИ УПРАВЛЕНИЯ ДОСТУПОМ К ИНФА\n\n"
...
@admin_labeler.message(text=["Залылоги", "залылоги"])
async def halls_logs_handler(message: Message):
    user_id = message.from_id
    ...
    logs = await get_admin_logs(log_type="halls", limit=50)
    
    if not logs:
        return "📭 Логи управления фитнес-залами отсутствуют!"
    
    logs_text = "🏢 ЛОГИ УПРАВЛЕНИЯ ФИТНЕС-ЗАЛАМИ\n\n"
...
async def make_admin_handler(message: Message, cmd_args: str):
    ...
    return (
        ...
        f"🆔 Админ ID: {admin_id}\n"
        f"👮 Назначил: [id{user_id}|{admin_nickname}]\n\n"
...
async def confirm_reset_all_handler(message: Message):
    ...
    return (
        ...
        f"💡 Для обновления статистики используйте команду:\n"
        f"Обновить статистику"
    )
...
async def create_promo_handler(message: Message, cmd_args: str):
    ...
    if reward_type not in ["монеты", "сила"]:
        return "❌ Неверный тип награды!\n✅ Допустимые типы: монеты, сила"
...
async def admin_clan_info_command(message: Message, tag: str):
    ...
    response_text = (
        ...
        f"👑 Владелец: {owner['username'] if owner else 'Не найден'} (ID: {clan['owner_id']})\n"
        ...
        f"🎯 Бонусы клана:\n"
        f" 💼 +{clan_bonuses['hall_income_bonus']}% дохода с фитнес-залов в казну\n"
...
async def admin_player_info_handler(message: Message, cmd_args: str):
    ...
    try:
        from bot.db import get_player_fitness_halls
        fitness_halls = await get_player_fitness_halls(target_id)
        daily_income_from_halls = fitness_halls * 10
        fitness_halls_text = f"🔸 Фитнес-залы: {format_number(fitness_halls)}\n"
        daily_income_text = f"🔸 Ежедневный доход с залов: {format_number(daily_income_from_halls)}\n"
    except:
        fitness_halls_text = ""
        daily_income_text = ""
    ...
    admin_level_target = target_player.get("admin_level", 0)
    if admin_level_target == 1:
        admin_status = "👑 Создатель"
    elif admin_level_target == 2:
        admin_status = "⭐ Старший администратор"
    elif admin_level_target == 3:
        admin_status = "👮 Модератор"
    else:
        admin_status = "👤 Игрок"

    info_text = (
        f"📊 Полная информация о игроке 📊\n"
        f"𝐆𝐘𝐌 𝐋𝐄𝐆𝐄𝐍𝐃\n\n"
        ...
        f"💰 Экономика:\n"
        f"🔸 Баланс: {format_number(target_player['balance'])}\n"
        f"{fitness_halls_text}"
        f"{daily_income_text}"
        f"🔸 Всего заработано: {format_number(target_player.get('total_earned', 0))}\n"
        f"🔸 Всего потрачено: {format_number(target_player.get('total_spent', 0))}\n\n"
        
        f"💪 Прогресс:\n"
        f"🔹 Сила: {format_number(target_player['power'])}\n"
        f"🔹 Гантеля: {target_player['dumbbell_name']} (Уровень: {target_player['dumbbell_level']})\n"
    ...
    if clan:
        info_text += (
            ...
            f"🛡️ Вклад в казну: {format_number(target_player.get('clan_contributions', 0))}\n"
        )
    
    info_text += f"\n📋 Административная информация:\n"
    info_text += f"👮 Уровень доступа администратора: {admin_level_target}\n"
    info_text += f"🆔 ID пользователя: {target_id}"
...
async def grant_info_access_handler(message: Message, cmd_args: str):
    """Выдать/отозвать доступ к команде Инфа (для обычных игроков)"""
    user_id = message.from_id
    ...
    if not await can_use_command(user_id, "donat_services"):
        return "❌ У вас нет доступа к управлению доступом!"
    ...
    if target_player.get("admin_level", 0) > 0:
        return f"❌ Игрок [id{target_id}|{target_username}] уже является администратором и имеет безграничный доступ к команде Инфа!"
...
async def list_info_access_handler(message: Message):
    ...
    return (
        f"📋 Игроки с доступом к команде Инфа:\n\n"
        f"Всего: {len(all_access)} игроков\n\n"
        f"{players_text}\n"
        f"💡 Администрация имеет безграничный доступ к команде Инфа\n"
...
async def delete_player_handler(message: Message, cmd_args: str):
    ...
    return (
        ...
        f"💪 Поднятий: {format_number(target_player['total_lifts'])}\n"
        f"🏢 Фитнес-залы: {format_number(await get_player_fitness_halls(target_id))}\n"
...
# ======================
# ОСТАЛЬНЫЕ ОСНОВНЫЕ КОМАНДЫ АДМИНИСТРАЦИИ
# ======================

@admin_labeler.message(text=["Аник <nickname>", "аник <nickname>"])
async def set_admin_nickname_handler(message: Message, nickname: str):
    user_id = message.from_id
    ...
    if not nickname or len(nickname) < 2:
        return "❌ Ник должен содержать минимум 2 символа!"
    
    if len(nickname) > 30:
        return "❌ Ник слишком длинный! Максимум 30 символов."
    
    await set_admin_nickname(user_id, nickname)
    
    await log_admin_action(
        user_id,
        "set_admin_nickname",
        user_id,
        f"Установил админ ник: {nickname}",
        None
    )
    
    return f"✅ Админ-ник установлен: {nickname}\n💡 Теперь вы будете отображаться как {nickname} в админ логах."

@admin_labeler.message(text=["Бан <cmd_args>", "бан <cmd_args>"])
async def ban_player_handler(message: Message, cmd_args: str):
    user_id = message.from_id
    ...
    try:
        days = int(parts[1])
        if days <= 0:
            return "❌ Дни бана должны быть положительным числом!"
        if days > 365:
            return "❌ Максимальный срок бана - 365 дней!"
    except ValueError:
        return "❌ Дни бана должны быть числом!"
    ...
    if target_player.get("is_banned", 0) == 1:
        return f'❌ Игрок "{target_username}" уже забанен!'
    
    await ban_player(target_id, user_id, days, reason)
    ...
    await log_admin_action(
        ...
        "ban_player",
        target_id,
        f"Забанил на {days} дней | Причина: {reason}",
        None
    )
    ...
    return (
        ...
        f"⏳ Срок: {days} дней\n"
        f"📝 Причина: {reason}\n"
        f"👮 Забанил: [id{user_id}|{admin_nickname}]"
    )

@admin_labeler.message(text=["Пермбан <cmd_args>", "пермбан <cmd_args>"])
async def permaban_player_handler(message: Message, cmd_args: str):
    user_id = message.from_id
    ...
    await ban_player(target_id, user_id, 36500, f"ПЕРМАНЕНТНО: {reason}")
    ...
    await log_admin_action(
        ...
        "permaban",
        target_id,
        f"Пермаментно забанил | Причина: {reason}",
        None
    )
...
@admin_labeler.message(text=["Разбан <user_id>", "разбан <user_id>"])
async def unban_player_handler(message: Message, user_id: str):
    ...
    try:
        target_id = int(pointer_to_screen_name(user_id))
    except ValueError:
        return "❌ Айди игрока должно быть числом!"
    ...
    if target_player.get("is_banned", 0) == 0:
        return f'❌ Игрок "{target_username}" не забанен!'
    
    await unban_player(target_id, admin_id)
    ...
    return (
        f"✅ Игрок разбанен!\n\n"
        f"👤 Игрок: [id{target_id}|{target_username}]\n"
        f"👮 Разбанил: [id{admin_id}|{admin_nickname}]"
    )
...
async def change_username_handler(message: Message, cmd_args: str):
    ...
    if len(new_username) > 50:
        return "❌ Ник слишком длинный! Максимум 50 символов."
    ...
    await update_username(target_id, new_username, user_id)
    ...
    await log_admin_action(
        ...
        "change_username",
        target_id,
        f"Сменил ник с '{old_username}' на '{new_username}'",
        None
    )
    
    return (
        f"✅ Ник игрока изменен!\n\n"
        f"👤 Игрок: [id{target_id}|{new_username}]\n"
        f"📝 Старый ник: {old_username}\n"
...
async def auto_cleanup_logs():
    ...
    while True:
        try:
            await asyncio.sleep(15 * 24 * 60 * 60)
