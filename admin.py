...
from bot.db import (
    ...
    get_all_info_access,
    extend_info_access,
    # Удаленные функции для донат бизнеса
...
def create_main_admin_keyboard(admin_level: int):
    ...
    # Ряд 3: Управление доступом Инфа (фиолетовый)
...
async def access_logs_handler(message: Message):
    ...
    # Получаем логи управления доступом
...
async def admin_player_info_handler(message: Message, cmd_args: str):
    ...
    # Администраторы имеют безграничный доступ к команде Инфа
    ...
    # Получаем количество фитнес-залов (если функция доступна)
    try:
        # Импортируем здесь чтобы избежать ошибок если функция не существует
    ...
    # Определяем уровень администратора если есть
...
async def ban_player_handler(message: Message, cmd_args: str):
    ...
    # Проверяем, не является ли администратором
    if target_player.get("admin_level", 0) > 0:
...
async def permaban_player_handler(message: Message, cmd_args: str):
    ...
    await ban_player(target_id, user_id, 36500, f"ПЕРМАНЕНТНО: {reason}")  # 100 лет ≈ пермамент
