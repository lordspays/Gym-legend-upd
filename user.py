...
from bot.utils import format_number, pointer_to_screen_name, convert_kkk_to_number
...
async def transfer_money_handler(message: Message, cmd_args: str):
    ...
    try:
        amount = convert_kkk_to_number(amount_str)
        if amount <= 0:
            return "❌ Сумма перевода должна быть положительным числом!"
    except ValueError:
        return "❌ Сумма перевода должна быть числом или в формате К,КК,ККК!"
