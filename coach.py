...
TRAINING_COOLDOWN = timedelta(hours=3)
    ...
    if last_training:
        ...
        if datetime.now() < next_training:
            ...
            cooldown_text = (
                ...
                f"🕐 Ближайшая тренировка через: {hours_left} часа {minutes_left} минут\n"
    ...
    try:
        ...
        if got_bonus:
            ...
            success_text = (
                ...
                f"⏰ Следующая тренировка через: 3 часа"
            )
        else:
            # Даем обычный доход
...
async def portfolio_handler(message: Message):
    ...
    if last_training:
        ...
        if datetime.now() < next_training:
            ...
            hours_left = time_left.seconds // 3600
            minutes_left = (time_left.seconds % 3600) // 60
            time_until_training = f"{hours_left} часа {minutes_left} минут"
            next_time = next_training.strftime("%H:%M")
        else:
            next_time = "Сейчас"
    else:
